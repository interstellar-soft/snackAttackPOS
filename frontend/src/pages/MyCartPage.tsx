...
export function MyCartPage() {
  ...
  const refundMutation = MyCartService.useRefundPurchase();
                      ...
                      <tr key={purchase.id} className="hover:bg-slate-100/60 dark:hover:bg-slate-800/60">
                        ...
                        <td className="px-3 py-2 text-right">
                          <button
                            ...
                            disabled={refundMutation.isPending || purchase.lineIds.length === 0}
                            onClick={async () => {
                              try {
                                await refundMutation.mutateAsync({
                                  transactionId: purchase.transactionId,
                                  lineIds: purchase.lineIds
                                });
                                window.alert(t('myCartRefundSuccess'));
                              } catch (error) {
                                const message = error instanceof Error ? error.message : t('myCartRefundError');
                                window.alert(message);
                              }
                            }}
                          >
                            {refundMutation.isPending ? t('loading') : t('myCartRefund')}
                          </button>
                        </td>
                      </tr>
                    );
