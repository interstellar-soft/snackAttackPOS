...
public class TransactionsControllerApiTests : IClassFixture<TransactionsApiFactory>
{
    ...
    public async Task Checkout_WithManualOverrides_RecordsOverrides()
    {
        ...
        var body = await response.Content.ReadFromJsonAsync<CheckoutResponse>();
        Assert.NotNull(body);
        Assert.True(body!.HasManualTotalOverride);
    ...
    [Fact]
    public async Task SettleDebt_AllowsPartialPayments()
    {
        var client = _factory.CreateClient();
        ...
        var settleBody = await settleResponse.Content.ReadFromJsonAsync<TransactionResponse>();
        Assert.NotNull(settleBody);
        Assert.True(settleBody!.BalanceUsd > 0m || settleBody.BalanceLbp > 0m);
        Assert.Null(settleBody.DebtSettledAt);
